...
class FlixHQ extends MovieParser {
  ...
  constructor() {
    super();
    this.name = "MyFlixHQ";
    this.baseUrl = "https://myflixerz.se";
    this.logo = "https://myflixerz.se/images/logo.png";
    ...
    try {
      ...
      $(".flw-item").each((i: number, el: any) => {
        const releaseDate = $(el)
          .find(".fd-infor .fdi-item:first-child")
          .text();
    ...
    try {
      ...
      $(".film_list-wrap .flw-item").each((i: number, el: any) => {
        recommendationsArray.push({
          ...
          image: $(el).find(".film-poster > img").attr("data-src") || "",
          duration:
            $(el).find(".fd-infor .fdi-duration").text().trim() || undefined,
          type: $(el)
            .find(".fd-infor .fdi-type")
            .text()
            .toLowerCase()
            .includes("tv")
            ? TvType.TVSERIES
      ...
      movieInfo.type = mediaId.includes("/movie/")
        ? TvType.MOVIE
        : TvType.TVSERIES;

      // Extract duration from the button
      const durationText = $(".btn-quality")
        .next('.btn-sm:contains("min")')
        .text()
        .trim();
      if (durationText) {
      ...
      if (movieInfo.type === TvType.TVSERIES) {
        const { data: seasonData } = await this.client.get(
          `${this.baseUrl}/ajax/season/list/${uid}`
        );
        ...
        for (const id of seasonsIds) {
          ...
          $$$(".nav > li").each((i: number, el: any) => {
            const episode = {
              ...
              season: season,
              url: `${this.baseUrl}/ajax/episode/servers/${$$$(el)
                .find("a")
                .attr("data-id")}`,
            };
  ...
  async fetchEpisodeSources(
    episodeId: string,
    mediaId: string,
    server: string = StreamingServers.VidCloud
    ...
    try {
      ...
      const { data } = await this.client.get(
        `${this.baseUrl}/ajax/sources/${servers[i].url
          .split(".")
          .slice(-1)
          .shift()}`
      );
  ...
  async fetchEpisodeServers(
    ...
    else episodeId = `${this.baseUrl}/ajax/movie/servers/${episodeId}`;
    ...
    try {
      ...
      return $(".nav li")
        .map((i: number, el: any) => ({
          ...
          url: `${this.baseUrl}/${mediaId}.${$(el)
            .find("a")
            .attr("data-id")}`.replace(
  ...
  async fetchMovieEmbedLinks(
    movieId: string,
    serverName: string | null = null
  ): Promise<any> {
    try {
      const { data: serverData } = await this.client.get(
      ...
      if (serverName) {
        const serverElement = $(".nav-item a").filter((i: number, el: any) => {
          return (
            $(el).find("span").text().toLowerCase() === serverName.toLowerCase()
          );
        });
          ...
          try {
            ...
            if (sourceData && sourceData.link) {
              ...
              try {
                ...
                console.error(
                  `Failed to extract direct link from ${serverName}:`,
                  err
                );
              }
            }
            ...
            console.error(
              `Failed to fetch source data from server ${serverName}:`,
              err
            );
          }
        }
  ...
  async fetchTvEpisodeEmbedLinks(
    episodeId: string,
    serverName: string | null = null
  ): Promise<any> {
    try {
